...
export async function POST(req: NextRequest) {
  try {
    ...
    const { message, sessionId = "default_session", model = "auto", history = [] } = body;
    ...
    let aiRes = await aiRouter.route(recentMsgs as any, systemPrompt, model);
    ...
    if (toolMatch) {
      ...
      aiRes = await aiRouter.route(recentMsgs as any, systemPrompt, model);
    }
