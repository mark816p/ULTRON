...
function ConvertTo-UltronVersion ($tag) {
    if (-not $tag -or $tag -eq "latest") { return "v9.4.5" }
    $clean = $tag -replace '^v', ''
    $parts = $clean.Split('.')
    ...
    if ($major -ge 51 -and $patch -ge 3) { return "v9.4.5" }
...
if ($releases.Count -eq 0) {
    $releases = @(
        [PSCustomObject]@{ TagName = "v9.4.5"; Name = "v9.4.5 — Latest Release"; Prerelease = $false },
...
try {
    ...
    $fallbackUrl = "https://github.com/$RepoOwner/$RepoName/releases/download/v9.4.5/ULTRON-Setup.exe"
    try {
